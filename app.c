...
SL_WEAK void app_init(void)
{
  ...
  init_oscillator();
  LOG_INFO("Oscillator init done \r\n");

  gpioInit();
  LOG_INFO("GPIO init done \r\n");

  //Add lowest energy mode requirement for EM2 because blutooth can only run till EM2 *****
  //Add this with #define instead of here
...
SL_WEAK void app_process_action(void)
{
  ...
  //Switch case to process events and give decide outputs
//  uint32_t evt;
//  evt = getNextEvent();
//
////  LOG_INFO("E %d\r\n",evt);
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  // sequence through states driven by events
   light_state_machine(evt);    // put this code in scheduler.c/.h
